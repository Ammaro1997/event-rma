...
<head>
    <title>
        <?php echo app('translator')->getFromJson("Attendee.check_in", ["event"=>$event->title]); ?>
    </title>

    <?php echo HTML::script('vendor/vue/dist/vue.min.js'); ?>

    <?php echo HTML::script('vendor/vue-resource/dist/vue-resource.min.js'); ?>


    <?php echo HTML::style('assets/stylesheet/application.css'); ?>

    <?php echo HTML::style('assets/stylesheet/check_in.css'); ?>
            ...
            <div class="col-md-12">
                <div class="attendee_input_wrap">
                    <div class="input-group">
                        ...
                        <?php echo Form::text('attendees_q', null, [
                ...
                <div class="attendee_list">
                    <h4 class="attendees_title">
                        <span v-if="!searchTerm">
                            <?php echo app('translator')->getFromJson("ManageEvent.all_attendees"); ?>
                        </span>
                        <span v-else v-cloak>
                            {{searchResultsCount}} <?php echo app('translator')->getFromJson("ManageEvent.result_for"); ?> <b>{{ searchTerm }}</b>
                        </span>
                    </h4>

                    <div style="margin: 10px;" v-if="searchResultsCount == 0 && searchTerm" class="alert alert-info"
                         v-cloak>
                        <?php echo app('translator')->getFromJson("ManageEvent.no_attendees_matching"); ?> <b>{{ searchTerm }}</b>
                    </div>
                    ...
                    <ul v-if="searchResultsCount > 0" class="list-group" id="attendee_list" v-cloak>
                        ...
                        >
                            <?php echo app('translator')->getFromJson("Attendee.name"); ?>: <b>{{ attendee.first_name }} {{ attendee.last_name }} </b> &nbsp; <span v-if="!attendee.is_payment_received" class="label label-danger"><?php echo app('translator')->getFromJson("Order.awaiting_payment"); ?></span>
                        <br>
                            <?php echo app('translator')->getFromJson("Order.reference"); ?>: <b>{{ attendee.order_reference + '-' + attendee.reference_index }}</b>
                        <br>
                            <?php echo app('translator')->getFromJson("Order.ticket"); ?>: <b>{{ attendee.ticket }}</b>
...
<div role="dialog" id="QrModal" class="scannerModal" v-show="showScannerModal" v-cloak>
    <div class="scannerModalContent">
        ...
        <div class="scannerButtons">
                    ...
                    <?php echo app('translator')->getFromJson("Attendee.scan_another_ticket"); ?>
                    </a>
        </div>
        ...
        <div class="ScanResultMessage">
                    ...
                    <span class="message" v-if="scanResultObject.status == 'success'">
                        <span class="uppercase"><?php echo app('translator')->getFromJson("Attendee.name"); ?></span>: {{ scanResultObject.name }}<br>
                        <span class="uppercase"><?php echo app('translator')->getFromJson("Attendee.reference"); ?></span>: {{scanResultObject.reference }}<br>
                        <span class="uppercase"><?php echo app('translator')->getFromJson("Attendee.ticket"); ?></span>: {{scanResultObject.ticket }}
                    </span>
                    <span v-if="isScanning">
                        <div id="scanning-ellipsis"><?php echo app('translator')->getFromJson("Attendee.scanning"); ?><span>.</span><span>.</span><span>.</span></div>
                    </span>
        </div>
...
<?php echo HTML::script('vendor/qrcode-scan/llqrcode.js'); ?>

<?php echo HTML::script('assets/javascript/check_in.js'); ?>

</body>
</html>
<?php /**PATH C:\xampp\htdocs\Attendize-master\resources\views/ManageEvent/CheckIn.blade.php ENDPATH**/ ?>